...
QUE ES PCA Y PORQUE VAMOS A USARLO.
...
heart_2["oldpeak_discretizado"] <- cut(heart$Oldpeak, breaks = c(-2,0,2,4,6,8,10), labels = c(0, 1, 2, 3,4,5))
...
Vamos fijarnos el análisis PCA anterior para ordenar dichas variables según su importancia en el modelo y de esta manera usar para el modelo de clasificación aquellas variables que maximicen el resultado.
...
Se aprecia que la precisión con trials es  82.6797 % que es mejor que sin trials que era 85.29%.
...
Utilizamos las variables discretizadas anteriormente para realizar gráficos más representativos:
...
CUANDO ES MEJOR UN MODELO DE REGRESION LOGISITICA O UNO DE CLASIFICACION
