...
Observando el gáfico la variable **RestingBP**  no sigue una distribución normal.
...
Se observa que los puntos suelen seguir la recta, menos en los lados.
...
Como hipótesis nula, se quiere saber si el azúcar en sangre con enfermedades cardíacas es igual en hombres que en mujeres.

Como hipótesis alternativa se quiere saber si el azúcar en sangre con enfermedades cardíacas es mayor en hombres que en mujeres.
...
Vamos a dividir el conjunto de los datos en un conjunto de entrenamiento (tendrá el 80% de los datos) y otro de prueba para validar el modelo (el 20% restante).
...
Asumiremos que la predicción del modelo es 1  si la probabilidad del modelo de regresión logística es superior o igual a 0.5 y 0 de lo contrario.
...
Para clasificar dicha variable utilizaremos todas las variables anteriores excepto la edad normalizada y la edad discreta. 
...
Además, se han obtenido una serie de reglas que el modelo ha acogido como decisivas para observar si una persona tiene riesgo de enfermedad del corazón. Todas estas se explican a continuación:
    ...
    3. Con una probabilidad del 87.8%, una persona con dolor de pecho ATA/TA y una máxima frecuencia cardíaca mayor que 150, no tendrá una enfermedad cardíaca.
    
    4. Una persona cuyo ST_Slope es Up, no tendrá una enfermedad del corazón con un 82.8%.
    
    5. Una persona con dolor de pecho ASY, una presión arterial menor o igual que 131, azúcar en sangre mayor que 0, tendrá una enfermedad cardíaca con una probabilidad de 98.1%.
...
Vamos a representar gráficamente el árbol obtenido de dos maneras diferentes:
...
Además discretearemos el resto de variables continuas para intentar mejorar el modelo anterior , ya que los modelos de clasificación se comportan mejor con variables categóricas. Este proceso también nos servirá de utilidad a la hora de hacer representaciones gráficas más representativas.
...
Variable **RestingBP**, Presión arterial:
...
```
Se aprecia que los verdaderos positivos son un 32.7% frente a un 5.6% de falsos positivos. Por lo tanto, tiene una alta sensibilidad.
Por otro lado, se observa que los verdaderos negativos es 49.7% frente a un 12.1% de falsos negativos, entonces se puede decir que tiene una alta especificidad.
...
A continuación y para completar el análisis gráfico vamos a realizar el análisis gráfico de las variables más relevantes en referencia al análisis PCA y si tienen riesgo de sufrir o no un ataque cardíaco en relación con la edad:
...
En concreto en el proceso de limpieza y preporcesado hemos realizado las siguientes tareas:
    ...
    - Realizado un estudio de los valores nulos.
...
Enlace Vídeo: https://drive.google.com/drive/folders/1QObrmV2T0MWDYwsA_mvcrQ-m5H7otG8p?usp=sharing
