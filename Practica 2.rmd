...
Para ello y después de análizar los distintos dataset disponibles en la plataforma kaggle, hemos optado por el conjunto de datos disponible en el siguiente enlace: https://www.kaggle.com/fedesoriano/heart-failure-prediction.
...
Se aprecia como la información es bastante dispar entre los diferentes componentes PC1 y PC2. Por lo tanto, decidimos no eliminar ningún campo del dataset ya que podemos considerar que cada uno nos puede dar cierta importancia.
...
Se discretiza esta variable para dividir la edad en rangos y mediante gráficos, poder ver comos e distribuye las características de enfermedades de corazón en función de la edad.
...
$$
Por el teorema del límite central, podemos asumir normalidad, puesto que tenemos una muestra de
tamaño grande n y se desea realizar un test sobre la media. Por tanto, aplicamos un test de
hipótesis de dos muestras sobre la media. Aplicaremos la distribución t, dado que no se conoce la
...
5. Una persona con dolor de epcho ASY, una presióna rterial menor o igual que 131, azúcar en sangre mayor que 0, tendrá una enfermedad cardíaca con una probabilidad de 98.1%.
...
Viendo todo esto, se puede decir que  una persona que no tiene dolor de angina inducida por el ejercicio, con azúcar en sangre menor o igual a 0 y una presión arterial menor o igual a 148, no tenderá a tener enfermedades cardíacas.
...
Se aprecia que el verdadero positivo  es 32% frente a un 0.62% de falsos positivos. Por lo tanto, tiene una alta sensibilidad.
<br>
...
Ahora, calculamos la precisón del modelo a partir de la matriz de confianza.
...
Se aprecia que la precisión con trials es 83.33% que es mejor que sin trials que era 85.29%.
...
Se aprecia que el verdadero positivo es 32.4% frente a un 0.59% de falsos positivos. Por lo tanto, tiene una alta sensibilidad. 
Por otro lado, se observa que los verdaderos negativos es 51% frente a un 10.8% de falsos negativos, entonces se puede decir que tiene una alta especificidad.
...
G1: La edad influye en el riesgo o no de sufrir una enfermedad cardíaca, de 60 a 80 años el riesgo de sufrir una enfermedad cardíaca es mayor.
G2: Los hombres tienen más posibilidades de sufrir una enfermedad cardíaca que las mujeres
G3:
G4:
G5:
G6:
